...
#   command_to_execute = [python_path, "-m", "conda", "create", "--name", name, "python=3.9", "-y"]
    ...
    f"wsl -- bash -c \"source {path_activate} {name} && {path_pip} install fvcore==0.1.5.post20220504\"",
...
def setup(default_install_path,args):
    ...
    command_inside_wsl = [python_path, lien_path] + sys.argv[3:12] +[name]
    ...
    print("command in first : ", command)
...
if __name__ == "__main__":
    if len(sys.argv) > 3 and sys.argv[1] == "setup":
        ...
        "input": sys.argv[3],
        "output": sys.argv[4],
        "subdivision_level": sys.argv[5],
        "resolution": sys.argv[6],
        "model": sys.argv[7],
        "predictedId": sys.argv[8],

        "sepOutputs": sys.argv[9],
        "chooseFDI": sys.argv[10],
        "logPath": sys.argv[11]
        }
